 ...
 * 2) Min dies
 * 3) Max dies
 * 4) Min dies tot el viatge
 * 5) Min cost viatge
 * 6) Max cost viatge
 * 7) Arestes extres (0 vol dir que fotrem un arbre)
 * 8) Maxim hotels per ciutat
 * 9) Nom problema
 */
...
int main(int argc, char** argv) {
    ...
    int minDiesViatge = atoi(argv[4]);
    int minCostViatge = atoi(argv[5]);
    int maxCostViatge = atoi(argv[6]);
    int extraEdges = atoi(argv[7]);
    int maxHotels  = atoi(argv[8]);
    cerr << maxHotels << endl;
    ...
    cout << "(define (problem " << argv[9] << ")" << endl;
    ++spaceLevel;
    printSpaces();
    ...
    endline = 1;
    for(set< pair<int,int> >::iterator it = E.begin(); it != E.end(); ++it) {
        cout << "(= (price c" << it->first+1 << " c" << it->second+1 << ") " << rand()%100 + 1 << ") ";
        if(endline%3 == 0) {
            cout << endl; printSpaces();
            endline = 1;
        }
    ...
    for(int i=0; i<numCiutats; ++i) {
        for(int j=0; j<H[i].size(); ++j) {
            cout << "(= (price-hotel h" << H[i][j]+1 << " c" << i+1 << ") " << rand()%100 + 1 << ") ";
            if(endline%3 == 0) {
                cout << endl; printSpaces();
                endline = 1;
            }
    ...
    cout << "(not-started) (= (min-days) " << minDies << ") (= (max-days) " << maxDies  
    << ") (= (spent-days) 0) (= (min-days-viatge) " << minDiesViatge << ") (= (cost-travel) 0)" << 
    "(= (min-cost-travel) " << minCostViatge << ") (= (max-cost-travel) " << maxCostViatge << " ) (= (sum-interest) 0)";
    cout << endl;
    ----spaceLevel;
    printSpaces();
    cout << ")" << endl;
    printSpaces();
    cout << "(:goal (and (>= (cost-travel) (min-cost-travel)) (<= (min-days-viatge) 0)))" << endl;
    printSpaces();
    cout << "(:metric minimize (cost-travel))" << endl;
    --spaceLevel;
    printSpaces();
    cout << ")" << endl;
